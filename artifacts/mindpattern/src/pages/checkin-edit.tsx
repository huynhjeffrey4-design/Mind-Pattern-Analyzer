...
import { useGetCheckin, getGetCheckinQueryKey, useUpdateCheckin, getListCheckinsQueryKey } from "@workspace/api-client-react";
...
export default function CheckinEdit() {
  ...
  const { data: checkin, isLoading } = useGetCheckin(id, { query: { enabled: !!id, queryKey: getGetCheckinQueryKey(id) } });
  ...
  useEffect(() => {
    if (checkin) {
      setMood(checkin.moodRating);
      setStress(checkin.stressLevel);
      setSleep(checkin.sleepHours);
      setWorkload(checkin.workloadLevel || 5);
      setExercised(checkin.exercised);
      setSocialized(checkin.socialized);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCheckin.mutate({
      ...
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: getGetCheckinQueryKey(id) });
  ...
  if (isLoading) return <div className="flex h-full items-center justify-center"><Loader2 className="w-8 h-8 animate-spin" /></div>;

  return (
    <div className="p-8 max-w-2xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-serif text-foreground">Edit Check-in</h2>
      </div>
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex gap-4">
              <Button type="button" variant="outline" onClick={() => setLocation("/history")} className="w-full" data-testid="button-cancel">Cancel</Button>
              <Button type="submit" disabled={updateCheckin.isPending} className="w-full" data-testid="button-submit-checkin">
                {updateCheckin.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
                Update Check-in
              </Button>
            </div>
          </form>
